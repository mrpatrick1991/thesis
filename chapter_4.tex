...
\end{equation}
 ...
 In addition to the measurements $\mathbf{D}$, knowledge of the mapping $\mathbf{F}$ may be incomplete, or based on uncertain experimental data. To describe the challenges associated with inverse problems precisely some definitions are needed.
...
\begin{figure}[p]
    \centering
    \includegraphics[width=1.1\textwidth]{figures/chapter_4/why_non_negative_is_good/fig.pdf}
...
The techniques developed so far have taken a difficult inverse problem and transformed it into a more tractable version. Preconditioning though linear transformations reduces the condition number of the problem dramatically, and the subsequent application of second-order regularization provides the rest of the needed stability to the problem. The bias introduced by regularization techniques, which is difficult to quantify, has been kept to a minimum through preconditioning and cross-validation, and chosen so as to gradually relax to smooth solutions in scenarios with high measurement noise. The addition of a non-negativity constraint ensures that solutions are always physical, by construction. What remains is to generate a body of test data and demonstrate that the combination of these techniques generates models which describe the assumed spectra in the test data well. The approach taken will be two-fold. First, simulated data for a set of different plausible forms for precipitating electron spectra will be generated, and then iterated over their respective parameter spaces. Results will be shown in plots to motivate an understanding of the behaviour of the inversion method, and different properties, such as non-negativity constraints, will be turned off an on to see changes in relative performance. Second, a large set of simulated data will be randomly generated, and the inversion method will be run and scored in aggregate based on the normalized mean squared error. The average mean-squared error in inversions across this data set will be used as a metric to show the positive effects of different properties of the inversion method.   

\begin{figure}[p]
    \centering
    \vspace{-1cm}
    \includegraphics[width=1.1\textwidth]{figures/chapter_4/synthetic_data_exp/fig_exp_100keV.pdf}
    \caption{Retrieval of exponential electron spectra (100 KeV folding factor) from synthetic X-ray data across different simulated count rates. Second order Tikhonov regularization, with and without non-negativity constraints, and zeroth order Tikhonov regularization are applied. Regularization strengths are chosen through cross validation.}
    \label{synthetic_data_exp}
\end{figure}

\begin{figure}[p]
    \centering
    \vspace{-1cm}
    \includegraphics[width=1.1\textwidth]{figures/chapter_4/synthetic_data_exp/fig_exp_250keV.pdf}
    \caption{Retrieval of exponential electron spectra (250 KeV folding factor) from synthetic X-ray data across different simulated count rates. Second order Tikhonov regularization, with and without non-negativity constraints, and zeroth order Tikhonov regularization are applied. Regularization strengths are chosen through cross validation.}
    \label{synthetic_data_exp}
\end{figure}
...
\begin{figure}[p]
    \centering
    \vspace{-1cm}
    \includegraphics[width=1.1\textwidth]{figures/chapter_4/synthetic_data_exp/fig_exp_400keV.pdf}
    \caption{Retrieval of exponential electron spectra (400 KeV folding factor) from synthetic X-ray data across different simulated count rates. Second order Tikhonov regularization, with and without non-negativity constraints, and zeroth order Tikhonov regularization are applied. Regularization strengths are chosen through cross validation.}
    \label{synthetic_data_exp}
\end{figure}
...
\begin{figure}[p]
    \centering
    \vspace{-1cm}
    \includegraphics[width=1.1\textwidth]{figures/chapter_4/synthetic_data_gauss/fig_gauss_50keV.pdf}
    \caption{Retrieval of gaussian electron spectra (50 KeV variance) from synthetic X-ray data across different simulated count rates. Second order Tikhonov regularization, with and without non-negativity constraints, and zeroth order Tikhonov regularization are applied. Regularization strengths are chosen through cross validation.}
    \label{synthetic_data_gauss}
\end{figure}

\begin{figure}[p]
    \centering
    \vspace{-1cm}
    \includegraphics[width=1.1\textwidth]{figures/chapter_4/synthetic_data_gauss/fig_gauss_100keV.pdf}
    \caption{Retrieval of gaussian electron spectra (100 KeV variance) from synthetic X-ray data across different simulated count rates. Second order Tikhonov regularization, with and without non-negativity constraints, and zeroth order Tikhonov regularization are applied. Regularization strengths are chosen through cross validation.}
    \label{synthetic_data_gauss}
\end{figure}
...
\begin{figure}[p]
    \centering
    \vspace{-1cm}
    \includegraphics[width=1.1\textwidth]{figures/chapter_4/synthetic_data_gauss/fig_gauss_150keV.pdf}
    \caption{Retrieval of gaussian electron spectra (150 KeV variance) from synthetic X-ray data across different simulated count rates. Second order Tikhonov regularization, with and without non-negativity constraints, and zeroth order Tikhonov regularization are applied. Regularization strengths are chosen through cross validation.}
    \label{synthetic_data_gauss}
\end{figure}
...
\begin{figure}[p]
    \centering
    \vspace{-1cm}
    \includegraphics[width=1.1\textwidth]{figures/chapter_4/synthetic_data_mono/fig_mono.pdf}
    \caption{Retrieval of monoenergetic electron spectra from synthetic X-ray data across different simulated count rates. Second order Tikhonov regularization, with and without non-negativity constraints, and zeroth order Tikhonov regularization are applied. Regularization strengths are chosen through cross validation.}
    \label{synthetic_data_gauss}
\end{figure}

\newpage

The data evaluated so far are based on a hypothesis about the types of electron spectra which might exist in nature. In reality, outside the basic constraints which have been discussed so far (positivity, smoothness), and in the absence of additional information, the spectra can have any form. In order to see how the inversion method designed in this chapter performs in general, a large body of synthetic data needs to be created, which extends the test models evaluated in the preceding figures.

There is a decision which has to be made about the amount of structure contained in the test spectra. A test electron spectra which consists entirely of random noise contains little useful information, and the methods used to retrieve it from simulated data will reflect this. On the other hand, a test spectrum with little structure and very smooth features will represent an ``easy'' test case for the inversion methods. Since the aim is to evaluate inversion methods with the smallest possible number of assumptions on the source electron spectra, we will assign a parameter to the body of generated test data which will correspond to the amount of structure contained. 

A set of random electron spectra is generated, and a low-pass filter is applied with a randomly selected cutoff frequency. The distributions for each bin in the random spectra are uniform, as is the distribution for the selected cutoff frequency. Measurement noise which is consistent with a randomly selected total count rate is added to the synthetic X-ray data. The total integrated counts chosen are uniformly distributed in powers of 10, from $10^5$ to $10^9$, which roughly represent the possible measurement range of the real data. 
